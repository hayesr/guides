...
```html
{% raw %}<span tooltip="{{item.v_total_vms}} VMs">{% endraw %}
```

This is where the `substitute` filter comes in handy. The above situation would then be resolved as:

```html
{% raw %}<span tooltip="{{ '[[number]] VMs'|translate|substitute:{number: item.v_total_vms} }}">{% endraw %}
```
...
```html
<span>
{% raw %}{{ ((magicVariable != null) ? "It's there" : "It's not there") | translate }}{% endraw %}
</span>
```
won't be collected correctly by `gulp gettext-extract`. The above should correctly be:
```html
